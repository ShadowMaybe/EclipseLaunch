...
tasks.jar {
    ...
    destinationDirectory.set(file("../EclipseLaunch/src/main/assets/components/lwjgl3/"))
    // Auto update the version with a timestamp so the project jar gets updated by Pojav
    doLast {
        val versionFile = file("../EclipseLaunch/src/main/assets/components/lwjgl3/version")
        versionFile.writeText(System.currentTimeMillis().toString())
    }
    from({
        configurations.getByName("resolvable").map {
            println(it.name)
